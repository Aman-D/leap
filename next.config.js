...
const pluginNames = glob
  .sync(path.join(__dirname, "node_modules", "task-leap-v1"))
  .map((plugin) => plugin.replace(path.join(__dirname, "node_modules"), ""))
  .map((plugin) => plugin.replace(/\//g, ""));

const withTM = require('next-transpile-modules')(pluginNames);
