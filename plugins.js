...
const plugins = glob.sync(path.join(__dirname, "node_modules", "task-leap-v1"));
plugins.map((plugin) => {
  const pluginName = plugin.replace(path.join(__dirname, "node_modules"), "").replace(/\//g, "");
  fs.mkdirpSync(path.join(__dirname, "tmp", "plugin", pluginName));
  const pluginPages = glob.sync(path.join(plugin, "pages", "*"));
  pluginPages.map((page) => {
    const pageName = page.replace(path.join(__dirname, "node_modules", pluginName, "pages"), "").replace(/\//g, "");
    fs.writeFileSync(
